...
def test_reporting_indexes_requests_and_metrics(tmp_path) -> None:
    ...
    requests_index = reporting.get_requests_index()
    metrics = reporting.get_dashboard_metrics()

    assert len(requests_index.index) == 1
    assert metrics["total_requests"] == 1
    assert metrics["validated_requests"] == 1
